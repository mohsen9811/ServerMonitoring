...
export function Sidebar({ collapsed, onToggle, onOpenSettings }: SidebarProps) {
  ...
  return <div className="flex h-full flex-col">
    <div className={cn("flex h-[68px] items-center border-b border-white/5 px-3", collapsed ? "justify-center" : "justify-between")}>
      {!collapsed && <div className="flex min-w-0 items-center gap-3"><div className="relative grid h-10 w-10 place-content-center rounded-2xl border border-cyan-400/15 bg-cyan-400/[.07] text-cyan-300"><TerminalSquare className="h-5 w-5" /><span className="absolute -bottom-1 -left-1 h-3 w-3 rounded-full border-[3px] border-[#0c111d] bg-emerald-400" /></div><div><div className="text-sm font-black tracking-tight text-slate-100">ServerPulse</div><div className="mt-0.5 font-mono text-[8px] uppercase tracking-[.24em] text-cyan-500/60">NOC console</div></div></div>}
      <button onClick={onToggle} className="grid h-8 w-8 place-content-center rounded-xl border border-white/5 bg-white/[.025] text-slate-600 transition hover:text-cyan-300" aria-label={collapsed ? "باز کردن منو" : "بستن منو"}>{collapsed ? <ChevronLeft className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}</button>
    </div>

    <div className={cn("space-y-2 border-b border-white/5 p-3", collapsed && "px-2")}>
      <button onClick={() => { setCurrentServer(null); onOpenSettings(); }} className={cn("flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-cyan-400/15 bg-cyan-400/[.07] text-xs font-bold text-cyan-200 transition hover:bg-cyan-400/[.12]", collapsed && "px-0")}><Plus className="h-4 w-4" />{!collapsed && "اتصال سرور"}</button>
      {!collapsed && <div className="relative"><Search className="absolute right-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-600" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="جستجو در سرورها…" className="h-9 w-full rounded-xl border border-white/5 bg-black/20 pr-9 pl-3 text-[11px] text-slate-300 outline-none placeholder:text-slate-700 focus:border-cyan-400/20" /></div>}
    </div>

    <div className="flex items-center justify-between px-3 pb-1 pt-3">{!collapsed && <span className="text-[9px] font-bold uppercase tracking-[.18em] text-slate-700">زیرساخت · {servers.length}</span>}<button onClick={() => setSortMode(value => !value)} className={cn("grid h-7 w-7 place-content-center rounded-lg text-slate-700 transition hover:bg-white/[.03] hover:text-slate-400", sortMode && "bg-cyan-400/[.06] text-cyan-400")} title="مرتب‌سازی"><ChevronsUpDown className="h-3.5 w-3.5" /></button></div>
    ...
    <div className="scrollbar-none flex-1 space-y-1 overflow-auto px-2 py-2">
      {filtered.map((server, index) => {
        ...
        return <motion.div layout key={server.id} className="relative">
          <button onClick={() => setCurrentServer(server.id)} title={collapsed ? server.name : undefined} className={cn("group relative flex w-full items-center gap-2.5 overflow-hidden rounded-2xl border px-2.5 py-2.5 text-right transition", active ? "border-cyan-400/15 bg-cyan-400/[.075]" : "border-transparent hover:border-white/5 hover:bg-white/[.025]", collapsed && "justify-center px-1")}>
            {active && <span className="absolute inset-y-3 right-0 w-0.5 rounded-full bg-cyan-300 shadow-[0_0_9px_#22d3ee]" />}
            <div className={cn("relative grid h-8 w-8 shrink-0 place-content-center rounded-xl border", active ? "border-cyan-400/15 bg-cyan-400/[.08] text-cyan-300" : "border-white/5 bg-black/20 text-slate-600")}><Server className="h-3.5 w-3.5" /><span className="absolute -bottom-0.5 -left-0.5 h-2 w-2 rounded-full border-2 border-[#0b101b] bg-emerald-400" /></div>
            {!collapsed && <><div className="min-w-0 flex-1"><div className={cn("truncate text-[11px] font-extrabold", active ? "text-slate-100" : "text-slate-400")}>{server.name}</div><div dir="ltr" className="mt-0.5 truncate text-right font-mono text-[9px] text-slate-700">{server.host}</div></div><div className="flex items-center gap-1">{sql && <Database className="h-3 w-3 text-violet-400/60" />}{iis && <Globe2 className="h-3 w-3 text-cyan-400/60" />}</div></>}
          </button>
